...
import pandas as pd
from scipy.sparse import csc_matrix, eye, diags, spdiags
...
def peak_preprocess(CC, threshold):
...
# filtering peaks with width less 5
    for r in range(C.shape[1]):
        ...
        if len(peak_st)>0:
            for l in range(len(peak_st)):
                if peak_ed[l]-peak_st[l]+2 < 5:
                    C[peak_st[l]:peak_ed[l]+1,r]=0 
        if max(C[:,r]) < 0.04:
            C[:,r] = 0
...
# preserving peak with the highest intensity in each m/z channel
    ...
    for r in range(C.shape[1]):
        ...
        if len(peak_st)>1:
            intensity = []
            for l in range(len(peak_st)):
                intensity.append(max(C[peak_st[l]:peak_ed[l], r]))
...
# refine the shape of gaussian peak
    for r in range(C.shape[1]):
        ...
        if len(peaks) > 1:
            ...
            peak_st, peak_ed = peak_find(C[:,r:r+1], threshold)
            gaussleftfit = 'no'
            gaussrightfit = 'no'
...
def tail_fix(CC, threshold):
    C = np.copy(CC)
    COM = com_find(C, threshold)
    peak_st, peak_ed = peak_find(C, threshold)
    for r in range(COM):
...
def peak_halfremove(CC, threshold):
...
def dishalfremove(CC, C_ed, threshold):
# 2 remove half peak and add mark
    C = np.copy(CC)

    dishalfdo = 'None'
    dishalfnum = []
    COM = com_find(C_ed, threshold)
    if COM > 1:
        fun = lambda x: x[1]-x[0]
        for r in range(COM):
            st = []
    ...
    if COM > 1:
        ...
        for m in range(COM-1):
            for n in range(m+1, COM):
                ...
                if np.abs(np.argmax(C[:,m]) - np.argmax(C[:,n])) > 3 and ol_degree > 0.95 and np.max(C[intenmax_idx])/np.max(C[intenmin_idx]) > 8:
                    C[:, intenmin_idx] = 0
                    testdo = 'Done'
                    removenum.append(intenmin_idx)
...
# filer components with spectra or chromatogram is none
    for i in range(COM):
        if np.all(St[i,:]==0) == True or np.all(C_ed[:,i]==0) == True:
            C[:,i] = 0
    return C
                ...
                if len(peaks) > 0:
                    ...
                    if gaussleftfit == 'yes' and gaussrightfit == 'yes':
                        ...
                        if len(peak_x) > 2 and np.abs(inten_max-lp) > 2 and np.abs(inten_max-rp) > 2:
                            ...
                            popt,pcov = curve_fit(gaussian,peak_x,peak_y,p0=[np.max(C),inten_max,s], maxfev=1000000)
                            for j in range(rp, C.shape[0],1):
...
def data_process(work_path, filename, dist, thres):
    ...
    y_DeepSeg, ind_st_DeepSeg, ind_en_DeepSeg = UNet4S(work_path, mat, model_size, distance=dist, threshold=thres)
...
def DeepPCR(work_path, modelpath, filename, dist, thres):
    ...
    x_seg_pre = np.zeros((len(ind_st_DeepSeg), 128, 800), dtype=float32)
    chrom_divid = []
    for i in range(len(ind_st_DeepSeg)): 
        test = np.zeros((ind_en_DeepSeg[i]-ind_st_DeepSeg[i], 800), dtype=float32)    
        test[:, mz_min:mz_max] = Xtest[int(ind_st_DeepSeg[i]):int(ind_en_DeepSeg[i]), :]

        test_br = np.zeros_like(test)

        for j in range(test.shape[1]):
            if all(item == 0 for item in test[:,j]) == False:
    ...
    restored_model = tf.keras.models.load_model(modelpath+'/model.h5', custom_objects={"R_squared": R_squared, "PW_loss": PW_loss}) 
    ...
    for i in range(len(ind_st_DeepSeg)):
        ...
        plt.subplot(212)
        plt.plot(C_0)
        plt.savefig(figure_savepath_c + '/' + 'num={}.png'.format(i))
        plt.close()
        plt.cla()
        plt.clf()

        threshold = 3e-3
        C = peak_preprocess(C_0, threshold)
        ...
        if np.all(C == 0) == True:
            ...
            C_out, halfdo = peak_halfremove(C_in, threshold)
            if halfdo == 'Done':
                C2 = np.zeros_like(C)
            ...
            C_out, testdo, lapdo, lapnum = testt(C_in, threshold)
            if testdo == 'Done':
                C4 = np.zeros_like(C)
            ...
            C_in = tail_fix(C_in, threshold)

            C_in_0 = np.copy(C_in)
            C_pre_0 = np.copy(C_in)

            COM = com_find(C_in, threshold)
            ...
            if lapdo == 'Done':
                if dislapdo == 'Done' or dishalfdo == 'Done':
                    ...
                    COM_C_pre_0 = com_find(C_pre_0, threshold)
                    if COM_C_pre_0 == 0:
                    ...
                    COM_C_in = com_find(C_in_0, threshold)
                    if COM_C_in == 0:
                        reserve = np.argmax([np.max(C_in[:,h]) for h in range(COM)])
                    ...
                    COM = com_find(C_pre, threshold)
                    peak_st, peak_ed = peak_find(C_pre, threshold)
                    if COM > 1:
            ...
            cos_sim = np.dot(TIC_resol, TIC) / (norm(TIC_resol)*norm(TIC))
            ...
            plt.subplot(311)
            plt.plot(TIC, label='origin_TIC')
            plt.legend()
            plt.subplot(312)
            plt.plot(TIC_resol, label='resolved_TIC')
            plt.legend()
            plt.subplot(313)
            if wayname == 'frr':
                tic_single = C_ed
            ...
            plt.plot(tic_single)
            plt.savefig(tic_savepath + '/' + 'num={}.png'.format(i))
            plt.close()
            plt.cla()
            plt.clf()
            ...
            plt.subplot(2,2,4)
            plt.plot(C_tic)
            plt.savefig(figure_savepath + '/' + 'num={}.png'.format(i))
            plt.close()
            plt.cla()
            plt.clf()
...
if __name__ == '__main__':
    ...
    work_path = 'C:/Users/ZNDX001/Documents/Python_Scripts/DeepResolution2-main/DeepResolution2'
    modelpath = 'C:/DeepEER2/model_out/23_04_18/SepConv/24'

    save_main = 'C:/Users/ZNDX001/Desktop/DeepCPR_testn2_1'
    ...
    for file in files:
        ...
        threshold = 3e-3
        peak_excel_single, peak_excel_seg, ms_single = DeepPCR(work_path, modelpath, filename, dist=3, thres=15)
