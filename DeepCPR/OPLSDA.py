...
#                     coMN[:, noN] = coVn
# 
#                 if np.isnan(modelDF.iloc[noN].at['R2Y'][0]) is False and modelDF.iloc[noN].at['R2Y'][0] < 0.01:
#                     modelDF.iloc[noN].at["Signif."] = "N4"
#                 else:
#                     if np.isnan(modelDF.iloc[noN].at['Q2'][0]) is False and modelDF.iloc[noN].at['Q2'][0] < rulThrN:
#                         modelDF.iloc[noN].at["Signif."] = "NS"
#                     else:
#                         modelDF.iloc[noN].at["Signif."] = "R1"
# 
#                 if autNcoL is True and modelDF.iloc[noN].at["Signif."] != "R1" and noN>1:
#                     breL = True
#                     break
#                 else:
#                     cMN[:, 0] = cVn
...
#                           , round(modelDF.iloc[1 + orthoI].at["R2Y"] * 100), "%) is above 1% and Q2Y ("
#                           , round(modelDF.iloc[1 + orthoI].at["Q2"] * 100), "%) is still above "
#                           , round(rulThrN * 100), "%.")
# 
#         poMN = poMN[:, 0:orthoI]
#         toMN = toMN[:, 0:orthoI]
...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.scatter(vips, range(len(COMS)), c="r")
    plt.yticks(range(len(COMS)), COMS, size=20)
    plt.xticks(size=20)
    plt.xlabel('VIP values')
    plt.grid(axis="y", linestyle='-.')
    plt.tight_layout()
    ...
    w_R2, b_R2 = scatter_linefit(x_train, R2_train, 100, R2_0[0])
    w_Q2, b_Q2 = scatter_linefit(x_train, Q2_train, 100, Q2_0[0])
    ...
    plt.suptitle('permutation test', fontsize=22)
    plt.xlabel('undisturbed proportions')
    plt.savefig("C:/Users/ZNDX001/Desktop/text/DeepCPR/picture/graphpad/permutation.tif", dpi=300, format="pdf")
    plt.show()
    ...
    plt.savefig("C:/Users/ZNDX001/Desktop/text/DeepCPR/picture/graphpad/frequency.tif", dpi=300, format="pdf")
    plt.show()
