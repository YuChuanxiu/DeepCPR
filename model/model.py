...
if __name__ == '__main__':
    ...
    plt.plot(history.history['R_squared'], label='R$^2$_training')
    plt.plot(history.history['val_R_squared'], label='R$^2$_validation')
    plt.legend()
    plt.savefig(savepath+'/ACC.tif', dpi=300)
    ...
    plt.legend()
    plt.show()
    plt.savefig(savepath+'/LOSS.tif', dpi=300)

    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    ax1.set_ylabel('R$^2$', size=15, color='g')
    ...
    plt.show()
    plt.savefig(savepath+'/train.tif', dpi=300)
